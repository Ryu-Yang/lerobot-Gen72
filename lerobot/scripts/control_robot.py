...
def run_policy(robot: Robot, policy: torch.nn.Module, hydra_cfg: DictConfig, run_time_s: float | None = None):
    ...
    while True:
        ...
        with (
            torch.no_grad(),  # 使用 no_grad() 以节省内存
            torch.autocast(device_type=device.type) 
            if device.type == "cuda" and hydra_cfg.use_amp 
            else nullcontext(),
        ):
            # 为每个观察添加批次维度
            for name in observation:
                observation[name] = observation[name].unsqueeze(0).to(device)  # 直接移到设备上
        ...
        robot.send_action(action.to("cpu"))  # 将动作移回CPU
